...
(0, component_1.VantComponent)({
    ...
    methods: {
        ...
        getRanges: function () {
            ...
            if (data.type === 'fulltime') {
                return [
                    ...
                    {
                        type: 'second',
                        range: [data.minSecond, data.maxSecond],
                    },
                ];
            }
        ...
        correctValue: function (value) {
            var data = this.data;
            // validate value
            var isFullTime = data.type === 'fulltime';
            var isDateType = data.type !== 'time' && !isFullTime;
            if (isDateType && !isValidDate(value)) {
                value = data.minDate;
            }
            else if (!isDateType && !value) {
                var minHour = data.minHour, minMinute = data.minMinute;
                if (isFullTime) {
                    value = "".concat(padZero(minHour), ":").concat(padZero(minMinute), ":00");
                }
                else {
            ...
            // time type
            if (!isDateType) {
                var _a = value.split(':'), hour = _a[0], minute = _a[1], second = _a[2];
                hour = padZero(range(hour, data.minHour, data.maxHour));
                minute = padZero(range(minute, data.minMinute, data.maxMinute));
                if (isFullTime) {
                    second = padZero(range(second, data.minSecond, data.maxSecond));
        ...
        onChange: function () {
            ...
            else if (data.type === 'fulltime') {
                var indexes = picker.getIndexes();
                value = "".concat(+originColumns[0].values[indexes[0]], ":").concat(+originColumns[1]
                    .values[indexes[1]], ":").concat(+originColumns[2].values[indexes[2]]);
            }
            else {
                var indexes = picker.getIndexes();
